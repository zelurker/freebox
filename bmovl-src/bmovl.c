...
#define MAX_PREV 10
	...
	if (f) {
		/* Ca, c'est un Ènorme bug mplayer apparemment.
		 * Si on affiche la liste dans un blit sÈparÈ, le bandeau d'info fait
		 * apparaitre des "dÈchets" sur la gauche de la liste.
		 * Supprimer le blit fait disparaitre les dÈchets, mais il n'est pas
		 * sensÈ toucher ‡ cette zone.
		 * Le + Ètrange c'est qu'un clear efface les dÈchets ! */
		int oldx,oldy,oldw,oldh;
		fscanf(f,"%d %d %d %d",&oldw,&oldh,&oldx,&oldy);
		fclose(f);
		oldx += oldw;
...
static int list(int fifo, int argc, char **argv)
{
	...
	// apparaitre des dÈchets autour de la liste. Ca ne devrait pas arriver.
	// Pour l'instant le meilleur contournement c'est Áa.
